...
! mwPointer stuff for mexing
      ...
      mwPointer mxGetPr
	  mwPointer NN_pr,nNodes_pr,n_thread_pr,dist_pr
      mwPointer m, n
...
! fortran subroutine arguments
	  ...
	  real*8 :: nNodes_r,n_thread_r
      integer :: nNodes,n_thread
	  character*80 msg
      logical debu
       
      debu = .true. ! .true. o .false. per attivare o disattivare il debug
	  if(debu) open(unit=66,file='log_dist.txt',status='unknown')
...
! call the computational subroutine.
      allocate(dist(nNodes,nNodes))
      call dist_node_node(NN,nNodes,n_thread,dist)
...
! Load the output 1 into a MATLAB array.
      ...
      if(debu) write(66,*) 'ho deallocato la matrice e ora chiudo il log_dist.txt'
      if(debu) close(66)
      return
      end
